<template>
  <div class="teek-palette">
    ...
    <div class="palette-description">
      <p>
        基础色板当前包含 {{ functionalColors.length + extendedColors.length }} 个主色调及衍生色阶，
    ...
    <div>
      <h2>色板生成工具</h2>
      <p>如果上面的色板不能满足你的需求，你可以选择一个品牌色，Teek 的色彩生成算法会为你生成完整的色板。</p>
...
import type { ColorPaletteProps } from "./widget/color-palette.vue";
...
const colorDefinitions = {
  ...
  core: [
    ...
    { name: "Info / 信息", desc: "信息提示，状态说明", cssVar: "--teek-color-info" },
  ] as ColorDefinition[],
...
const generateShades = (baseColor: string, levelCount = 10) => {
  const shades: { level: number; value: string }[] = [];
  ...
  // 确保色阶按正确的顺序排列
  shades.sort((a, b) => a.level - b.level);
