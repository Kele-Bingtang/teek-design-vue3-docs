 ...
 * 对 model 对象的 pro 赋值
 */
export const setProp = (model: Record<string, any>, prop: NonNullable<FormItemColumnProps["prop"]>, value: unknown) => {
  if (!model) return;
...
export const findItemNested = (
  ...
  return options.reduce<Record<string, any> | null>((accumulator, current) => {
 ...
 */
export const filterOptionsValue = (
 ...
 * @param prefix 当前路径前缀
 */
export const getObjectKeys = (model: Record<string, any>, prefix = ""): string[] => {
