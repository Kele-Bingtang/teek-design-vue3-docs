...
## 线上动态修改环境变量 <Badge type="tip" text="v1.2.1" />

在默认情况下，`.env` 相关文件的环境变量在构建时会直接赋值到代码中，当发布线上环境时，如果我们需要修改部分环境变量，比如请求接口的域名，那么只能修改 `.env` 文件然后重新打包，对于庞大的项目来说，一次打包是非常耗时的。

因此 Teek 提供了线上动态修改环境变量功能，当打完包后，Teek 在 dist 目录下新增一个 `envConfig.js` 文件，您可以在里面提供一个和 `.env` 文件里相同命名的环境变量，Teek 会将这个变量覆盖掉 `.env` 文件里的变量，从而实现线上动态修改环境变量的功能。
...
当然您也可以在项目的 `public` 下直接看到 `envConfig.js` 文件，在项目构建时，该文件原封不动的输出到 `dist` 目录下，因此你也可以事先在本地修改该文件内容。

### 功能背景

Teek 团队在公司 A 基于 `Teek Design Vue3` 模板开发一个项目，一次偶然的机会，该项目被要求去公司 B 进行部署使用，但因为项目源码受到公司 A 信息安全部门要求，无法泄露源码，因此只能提前打包到公司 B 进行部署，因为公司 B 的接口地址、认证地址等与公司 A 不同，因此为了适配公司 B 的环境，团队在修改、打包、验证频繁来回，因此就有了该功能的诞生。

团队有了该功能后，在公司 B 部署出现问题就可以马上修改调试，当部署完毕且没问题后，团队回到公司 A 再对项目环境变量进行最终版的修改，最后交付一个完美适配公司 B 环境的部署包。

该功能是一个临时却非常及时有效的解决方案，适合「**救火**」，比如在生产环境出现异常时，临时修改环境变量，然后刷新页面，从而解决异常。但是最终还是在项目里进行最终版的修复。

### 原理和使用

线上动态修改环境变量的原理非常简单，在加载项目的 `index.html` 文件时，Teek 会将 `.env` 文件（`import.meta.env`）的环境变量注入到 `window.__TEEK_ENV__` 中，然后再读取 `envConfig.js` 文件，将该文件中的环境变量（如果设置）覆盖掉 `window.__TEEK_ENV__` 中的环境变量，从而实现线上动态修改环境变量的功能。

当然前提是您必须在项目里使用 `window.__TEEK_ENV__` 代替 `import.meta.env` 获取环境变量。

```javascript
import axios from "axios";

// 域名取自 window 下的环境变量，可以通过对 window 下的环境变量进行修改，从而实现线上动态修改环境变量的功能
axios.get(`${window.__TEEK_ENV__.VITE_API_URL}/api/user`).then(res => {});
...
::: tip
你可以全局修改 `__TEEK_ENV__` 的命名，比如修改为你的项目名，如 OA 系统：`__OA_ENV__`。
:::
