...
| tooltip              | 表头右侧 `ElToolTip` 提示                                                                                               | `string` / `object` <Tip content="{icon?: Component, render?: () => RenderTypes, contentRender?: () => RenderTypes} & UseTooltipProps" /> | —                                                                                                |
...
| elSlots              | `el` 组件的插槽                                                                                                         | `object` <Tip content="{ [slotName: string]: (data: TableRenderParams & Record<string, any>) => RenderTypes }" />                         | —                                                                                                |
...
| tooltipProps                         | `ElTable Props`                                                                                                         | `object` <Tip content="Partial<UseTooltipProps>" />                                                         | —         |
...
| headerResize <Badge text="v2.2.2" /> | 是否允许表格表头列宽度、高度拖拽调整大小（当 border 为 false 仍然支持调整列宽度大小）                                   | `boolean`                                                                                                   | false     |
