...
| render               | 自定义单元格内容渲染（`tsx` 语法）                                                                                      | `function` <Tip content="(scope: RenderParams<T>) => RenderTypes" />                                                                     | —                                                                                                |
| renderHTML           | 自定义单元格内容渲染（返回 `HTML` 字符串）                                                                              | `function` <Tip content="(scope: RenderParams<T>) => string" />                                                                          | —                                                                                                |
| formatValue          | 自定义单元格内容格式化                                                                                                  | `function` <Tip content="(value: unknown, scope: RenderParams<T>) => string \| number" />                                                | —                                                                                                |
| renderHeader         | 自定义表头内容渲染（tsx 语法）                                                                                          | `function` <Tip content="(scope: RenderParams<T>) => RenderTypes" />                                                                     | —                                                                                                |
| renderHeaderHTML     | 自定义表头内容渲染（返回 `HTML` 字符串）                                                                                | `function` <Tip content="(scope: RenderParams<T>) => string" />                                                                          | —                                                                                                |
| formatLabel          | 自定义表头内容格式化                                                                                                    | `function` <Tip content="(label: unknown, scope: RenderParams<T>) => string \| number" />                                                | —                                                                                                |
...
| beforeFilter         | 过滤前置处理，可以返回新的过滤值或 `false` 阻止过滤                                                                     | `function` <Tip content="(filterValue: unknown, filterModel: Record<string, any>) => unknown \| false \| undefined" />                   | false                                                                                            |
...
| elSlots              | `el` 组件的插槽                                                                                                         | `object` <Tip content="{ [slotName: string]: (data: RenderParams) => RenderTypes }" />                                                   | —                                                                                                |
...
| requestError           | 请求失败回调                                                                                                            | `function` <Tip content="(error: unknown) => void" />                                                       | —         |
...
| filter           | 过滤事件，返回输入的值以及 `prop`                                                                     | `function` <Tip content='(filterModel: Record<string, any>, filterValue: unknown, prop: string) => void' />         |
| filterClear      | 清空事件，返回输入的 `prop`                                                                           | `function` <Tip content='(prop: string) => void' />                                                                 |
| filterReset      | 重置所有表单事件                                                                                      | `function` <Tip content='() => void' />                                                                             |
| formChange       | 表单值改变事件                                                                                        | `function` <Tip content='(fromValue: unknown, prop: string, scope: TableScope) => void' />                          |
...
| 'prop'              | 自定义表格单元格，自动根据配置项的 `prop` 生成对应的插槽                                              | `object` <Tip content="params: RenderParams" />                                                                  |
| 'prop-header'       | 自定义表格项 `header`，自动根据配置项的 `prop` 生成对应的插槽                                         | `object` <Tip content="params: RenderParams" />                                                                  |
