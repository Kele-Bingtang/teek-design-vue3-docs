...
| defaultValue    | 表单属性的默认值                                                                                                        | `unknown` <Tip content="支持函数、Promise、Ref" />                                                                                                            | —                                                                                                |
...
| renderLabel     | 自定义 `label` 标题                                                                                                     | `function` <Tip content="(label: string, scope: FormItemColumnProps) => RenderTypes" />                                                                       | —                                                                                                |
| renderLabelHTML | 自定义 `label` 内容渲染（返回 HTML），优先级低于 render，高于插槽                                                       | `function` <Tip content="(label: string, scope: FormItemColumnProps) => string" />                                                                            | —                                                                                                |
| render          | 自定义渲染 `ElFormItem` 下的表单组件                                                                                    | `function` <Tip content="(value: unknown, update: (value: unknown) => void, scope: FormItemColumnProps) => RenderTypes" />                                    | —                                                                                                |
...
| change      | 表单变化触发的事件                                                                                     | `function` <Tip content='(model: unknown, model: Record<string, any>, column: FormItemColumnProps) => void' /> |
