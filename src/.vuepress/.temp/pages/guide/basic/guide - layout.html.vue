...
<p>Admin 的布局组件有顶栏、面包屑、侧边菜单栏、标签栏、内容区，位于 <code v-pre>src/layout</code> 下，分别位为 components 目录的 Header、Menu、TabsNav、MainContent 下。</p>
<p>内容区（MainContent）根据路由进行组件的跳转，可视化页面的组件在 <code v-pre>src/views</code> 下。</p>
<p>布局支持 5 种形式动态切换，分别为</p>
<ul>
...
<p>标签页支持两种形式，一种是保留了 <a href="https://vue2-admin.youngkbt.cn/" target="_blank" rel="noopener noreferrer">Vue2 Admin<ExternalLinkIcon/></a> 的经典版，另一种是使用了 Element Plus 提供的 <code v-pre>el-tabs</code> 组件。也采用了 components 内置组件来动态切换，具体请看 <code v-pre>src/layout/components/TabsNav</code> 内容。</p>
<blockquote>
<p>默认布局</p>
</blockquote>
...
<p>Admin 继承了 Vue3 的核心思想：hooks 函数，位于 <code v-pre>sc/hooks</code> 下。</p>
</div></template>
